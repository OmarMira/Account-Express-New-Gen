...
describe('roleIsValidForDirection', () => {
  ...
  describe('with pure credit profile (creditPct=0.85, debitPct=0.10)', () => {
    const creditProfile = { creditPct: 0.85, debitPct: 0.10 };
  ...
  describe('with pure debit profile (creditPct=0.10, debitPct=0.90)', () => {
    const debitProfile = { creditPct: 0.10, debitPct: 0.90 };
  ...
  // ── Ambas (mixed) profile ───────────────────────────────────────
  describe('with mixed profile (creditPct=0.50, debitPct=0.50)', () => {
    const mixedProfile = { creditPct: 0.50, debitPct: 0.50 };
  ...
  describe('bypass roles', () => {
    ...
    it('allows SOCIO with ambas', () => {
      expect(roleIsValidForDirection('SOCIO', { creditPct: 0.50, debitPct: 0.50 })).toEqual({ valid: true });
    });
  ...
  describe('threshold boundary at 0.80 (80%)', () => {
    it('treats 0.79 credit as ambas (not pure credit)', () => {
      const result = roleIsValidForDirection('PROVEEDOR', { creditPct: 0.79, debitPct: 0.21 });
      // At 0.79 it's "ambas" — PROVEEDOR (debit) should be allowed
      expect(result).toEqual({ valid: true });
    });

    it('treats 0.80 credit as pure credit', () => {
      const result = roleIsValidForDirection('PROVEEDOR', { creditPct: 0.80, debitPct: 0.20 });
      // At 0.80 it's pure credit — PROVEEDOR (debit) should be rejected
      expect(result).toMatchObject({ valid: false });
      expect(result.reason).toBeTruthy();
    });

    it('treats 0.79 debit as ambas (not pure debit)', () => {
      const result = roleIsValidForDirection('CLIENTE', { creditPct: 0.21, debitPct: 0.79 });
      // At 0.79 it's "ambas" — CLIENTE (credit) should be allowed
      expect(result).toEqual({ valid: true });
    });

    it('treats 0.80 debit as pure debit', () => {
      const result = roleIsValidForDirection('CLIENTE', { creditPct: 0.20, debitPct: 0.80 });
      // At 0.80 it's pure debit — CLIENTE (credit) should be rejected
      expect(result).toMatchObject({ valid: false });
      expect(result.reason).toBeTruthy();
    });
  });
